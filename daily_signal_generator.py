#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Finspark AI v6.4.1 FINAL - CTO CERTIFIED + GAP FIX"""
...
logger=logging.getLogger("FinsparkV6.4.1")
if not SUPABASE_URL or not SUPABASE_KEY:logger.critical("❌ Missing SUPABASE credentials");sys.exit(1)
...
print("="*70)
print("🏆 FINSPARK AI v6.4.1 FINAL - CTO CERTIFIED")
print("="*70)
print(f"📅 {datetime.now(IST).strftime('%Y-%m-%d %H:%M:%S IST')}")
print(f"📊 Stocks:{len(STOCKS)}|Workers:{MAX_WORKERS}|Table:{TABLE_NAME}")
print(f"🎯 SL:{SL_ATR_MULT}x|TP:{TP_ATR_MULT}x|Vol:{VOL_SPIKE_MULT}x|MaxGaps:{MAX_INTRADAY_GAPS}")
print("="*70)
def is_market_open():
...
try:supabase=create_client(SUPABASE_URL,SUPABASE_KEY);logger.info("✅ Supabase connected")
except Exception as e:logger.critical(f"❌ Supabase failed:{e}");sys.exit(1)
def safe_iloc(series,idx,fallback=np.nan):
 if not isinstance(series,pd.Series)or series.empty:return fallback
 try:
  if idx<0 and abs(idx)>len(series):return fallback
  if idx>=0 and idx>=len(series):return fallback
  val=series.iloc[idx]
...
def get_value(series,idx):
 ...
 try:float_val=float(val);return float_val if float_val>0.01 else None
 except:return None
def validate_data_quality(data):
 ...
 if len(data)>1:
  ...
  if intraday_gaps>MAX_INTRADAY_GAPS:return False,f"Gaps({intraday_gaps})"
 last_time=data.index[-1]
 if last_time.tzinfo is None:last_time=last_time.tz_localize('UTC').tz_convert(IST)
 else:last_time=last_time.tz_convert(IST)
...
def calculate_indicators(df):
 ...
 return{'rsi':rsi,'atr':atr,'macd_hist':macd_hist,'bb_upper':bb_upper,'bb_lower':bb_lower,'vol_avg':vol_avg,'ema_20':ema_20,'ema_50':ema_50}
def fetch_data(ticker,max_retries=MAX_RETRIES):
 for attempt in range(1,max_retries+1):
  try:
   if attempt>1:delay=RETRY_DELAY*(2**(attempt-2))+random.random()*0.2;time.sleep(delay)
...
def determine_trend(ema_20,ema_50,close_price):
 if ema_20>ema_50*(1+EMA_TREND_BUFFER)and close_price>ema_20:return'Uptrend'
 elif ema_20<ema_50*(1-EMA_TREND_BUFFER)and close_price<ema_20:return'Downtrend'
 return'Sideways'
...
def generate_signal(stock_symbol,stock_num=0,total=0):
 ...
 try:
  ...
  today_open=get_value(data['Open'],last_idx)
  if close_price is None or close_price<=0:logger.debug(f"{prefix}❌ {pretty}:Invalid price");return None
  if today_open and today_open not in(None,0):
   circuit_pct=abs(close_price-today_open)/today_open
  ...
  if trend=='Uptrend':buy_votes+=1;strength_score+=3;reasons.append("Uptrend")
  elif trend=='Downtrend':sell_votes+=1;strength_score+=3;reasons.append("Downtrend")
  ...
  if volume and vol_avg:
   ...
   if vol_ratio>=VOL_SPIKE_MULT:
    if close_price>open_price:buy_votes+=1;strength_score+=3;reasons.append(f"Vol({vol_ratio:.1f}x)")
    elif close_price<open_price:sell_votes+=1;strength_score+=3;reasons.append("VolDump")
    else:hold_votes+=1
  ...
  if buy_votes>=MIN_VOTES_FOR_ACTION and trend!='Downtrend':signal='Buy';base_conf+=min(strength_score,50)+(buy_votes*5)
  elif sell_votes>=MIN_VOTES_FOR_ACTION and trend!='Uptrend':signal='Sell';base_conf+=min(strength_score,50)+(sell_votes*5)
  elif buy_votes==4 and trend=='Uptrend'and strength_score>=MIN_STRENGTH_SCORE:signal='Buy';base_conf+=min(strength_score,40)+(buy_votes*4)
  elif sell_votes==4 and trend=='Downtrend'and strength_score>=MIN_STRENGTH_SCORE:signal='Sell';base_conf+=min(strength_score,40)+(sell_votes*4)
  ...
  if signal!='Hold'and atr_val>0:
   risk=atr_val*SL_ATR_MULT;reward=atr_val*TP_ATR_MULT
  ...
  return result
 except Exception as e:logger.error(f"{prefix}❌ {pretty}:{e}");return None
def upload_signals(signals,table_name):
 try:
  if not signals:return 0
  seen=set();unique_signals=[]
  for s in signals:
   if s.get("confidence",0)<MIN_CONF_TO_UPLOAD:continue
   key=(s["symbol"],s["interval"],s["signal_date"],s["signal_time"])
   if key not in seen:seen.add(key);unique_signals.append(s)
  if not unique_signals:return 0
  uploaded=0
  for i in range(0,len(unique_signals),BATCH_SIZE):
   ...
   try:
    ...
    if resp.get('error'):logger.error(f"Supabase error:{resp.get('error')}");continue
    else:data=getattr(resp,'data',None)
    count=len(data)if data else len(batch)
    uploaded+=count
    logger.info(f"✅ Batch{i//BATCH_SIZE+1}:{count}/{len(batch)}")
   except Exception as e:logger.error(f"Batch{i//BATCH_SIZE+1}failed:{e}");continue
  return uploaded
 except Exception as e:logger.error(f"Upload failed:{e}");return 0
def main():
 logger.info("🚀 Starting v6.4.1 FINAL...")
 market_open,status=is_market_open()
 if not market_open:logger.info(f"⏸ Market closed({status}).Exiting.");sys.exit(0)
 if CLEAR_TODAY:
  try:today=datetime.now(IST).date().isoformat();logger.info(f"⚠️ Deleting old signals({today})...");delete_resp=supabase.table(TABLE_NAME).delete().eq('signal_date',today).execute();deleted_count=len(delete_resp.data)if hasattr(delete_resp,'data')and delete_resp.data else 0;logger.info(f"✅ Deleted{deleted_count}")
  except Exception as e:logger.warning(f"⚠️ Delete failed:{e}")
 ...
 with ThreadPoolExecutor(max_workers=MAX_WORKERS)as executor:
  ...
  for future in as_completed(future_to_stock):
   ...
   except signal:results.append(signal)
 ...
 elapsed=time.time()-START_TS;success=len(results);failed=len(failed_tickers)
 print("\n"+"="*70)
 print("📊 FINSPARK v6.4.1 FINAL SUMMARY")
 print("="*70)
 ...
 if results:
  ...
  if len(s_tier)>0:
   ...
   for _,row in s_tier.nlargest(5,'confidence').iterrows():emoji="🟢"if row['signal']=='Buy'else"🔴";print(f" {emoji} {row['symbol']:12s}{row['confidence']:.0f}% ₹{row['close_price']:.2f}→T:₹{row['target']} R:R{row['risk_reward']}")
  if len(a_tier)>0:
   print("\n🥇 A-TIER(Conf≥78%,R:R≥2.0):")
   for _,row in a_tier.nlargest(3,'confidence').iterrows():emoji="🟢"if row['signal']=='Buy'else"🔴";print(f" {emoji} {row['symbol']:12s}{row['confidence']:.0f}% ₹{row['close_price']:.2f}→T:₹{row['target']} R:R{row['risk_reward']}")
  elif len(hq_buys)>0:
   print(f"\n🔥 TOP BUY(Conf>70%,R:R≥{MIN_RR_TO_SHOW}):")
   for _,row in hq_buys.nlargest(3,'confidence').iterrows():print(f" 🟢 {row['symbol']:12s}{row['confidence']:.0f}% ₹{row['close_price']:.2f}→T:₹{row['target']} R:R{row['risk_reward']}")
  if len(hq_sells)>0:
   print(f"\n🔥 TOP SELL(Conf>70%,R:R≥{MIN_RR_TO_SHOW}):")
   for _,row in hq_sells.nlargest(3,'confidence').iterrows():print(f" 🔴 {row['symbol']:12s}{row['confidence']:.0f}% ₹{row['close_price']:.2f}→T:₹{row['target']} R:R{row['risk_reward']}")
