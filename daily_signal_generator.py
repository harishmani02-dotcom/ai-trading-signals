...
CTO Certified + All Optimizations:
- EMA angle/momentum filter
- MACD divergence detection
- Stricter volume + candle confirmation
- Optimized R:R (2.5:1)
- Higher quality thresholds (70% conf, 2.0 R:R)
"""
...
MIN_CONF_TO_UPLOAD=float(os.getenv("MIN_CONF_TO_UPLOAD","70"))
...
if not SUPABASE_URL or not SUPABASE_KEY:logger.critical("❌ Missing SUPABASE");sys.exit(1)
...
def calculate_indicators(df):
 ...
 return{'rsi':rsi,'atr':atr,'macd_hist':macd_hist,'bb_upper':bb_upper,'bb_lower':bb_lower,'vol_avg':vol_avg,'ema_20':ema_20,'ema_50':ema_50,'close':close}
...
def determine_trend_with_momentum(ema_20_series,ema_50,close_price):
 ema_20_current=safe_iloc(ema_20_series,-1)
 ema_20_prev=safe_iloc(ema_20_series,-5)
...
def detect_macd_divergence(close_series,macd_hist_series):
 ...
 try:
  price_last=safe_iloc(close_series,-1);price_prev=safe_iloc(close_series,-5)
  macd_last=safe_iloc(macd_hist_series,-1);macd_prev=safe_iloc(macd_hist_series,-5)
...
def main():
 ...
 if CLEAR_TODAY:
  try:
   ...
   logger.info(f"⚠️ Deleting({today})...")
